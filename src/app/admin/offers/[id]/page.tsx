import Link from "next/link";
import { notFound } from "next/navigation";
import { ChevronLeft } from "lucide-react";
import { getOfferById } from "@/lib/db";
import { updateOffer } from "@/app/actions/offers";
import OfferForm from "@/components/admin/OfferForm";

export default async function EditOfferPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const offer = await getOfferById(Number(id));

  if (!offer) notFound();

  return (
    <div className="p-8 md:p-12 max-w-3xl">
      <Link
        href="/admin/offers"
        className="flex items-center gap-1.5 text-[11px] uppercase tracking-widest font-bold text-gray-400 hover:text-[#2c2c2c] transition-colors mb-8"
      >
        <ChevronLeft size={14} /> Back to Offers
      </Link>

      <div className="mb-8">
        <h2 className="text-2xl font-serif text-[#2c2c2c] tracking-tight">Edit Offer</h2>
        <p className="text-gray-400 text-sm mt-1 font-mono">#{offer.id}</p>
      </div>

      <OfferForm offer={offer} action={updateOffer} submitLabel="Save Changes" />
    </div>
  );
}
